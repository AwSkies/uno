...
public class Hand {
    ...
    public Hand(UnoPlayer player) {
        ...
        this.playerName = player.getClass().getSimpleName();
    ...
    Card play(Game game) {
        int playedCard;
        int x = 0;
        playedCard = player.play(new ArrayList<Card>(cards), game.getUpCard(), game.calledColor,
            game.getGameState());
        if (playedCard == -1) {
            return null;
        }
